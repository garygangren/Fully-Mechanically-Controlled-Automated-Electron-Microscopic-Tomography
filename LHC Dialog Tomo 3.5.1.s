Class LHC_TomoDialog: UIFrame
{
	...
	number m_RefDefocus
	string m_ImgDefocus
	number m_AcqShiftOn, m_AcqShift
	...
	number m_Delay
	number m_kpp, m_kip
	number m_PositionErrorThreshold, m_PositionErrorWarningOn
	number m_wDiv, m_hDiv, m_nr, m_Cs, m_AC
	number m_DefocusMin, m_DefocusMax, m_DefocusStep, m_DefocusAccuracy
	number m_kpf, m_kif
	number m_DefocusErrorThreshold, m_DefocusErrorWarningOn
	// control parameters
	...
	object Init(object self)
	{
		...
		if(name.DoesFileExist())
		{
			...
			if(file.ReadFileLine(str))	m_AcqShiftOn = val(str)
			if(file.ReadFileLine(str))	m_AcqShift = val(str)
		...
		// group acquiring shift
		TagGroup Group_AcqShift = DLGCreateGroup().DLGIdentifier("#Group_AcqShift")
		Group_AcqShift.DLGAddElement(DLGCreateLabel("Acquiring shift:",15).DLGAnchor("West"))
		Group_AcqShift.DLGAddElement(DLGCreateCheckBox("",m_AcqShiftOn).DLGIdentifier("#Check_AcqShiftOn"))
		Group_AcqShift.DLGAddElement(DLGCreateIntegerField(m_AcqShift*1e9).DLGWidth(16).DLGIdentifier("#Field_AcqShift"))
		Group_AcqShift.DLGAddElement(DLGCreateLabel("nm",4).DLGAnchor("West"))	
		// group track
		...
		dialog.DLGAddElement(Group_AcqShift.DLGTableLayOut(4,1,0))
	...
	void AboutToCloseDocument(object self,number verify )
	{
		...
		m_AcqShiftOn = self.LookUpElement("#Check_AcqShiftOn").DLGGetValue()
		m_AcqShift = self.LookUpElement("#Field_AcqShift").DLGGetValue()*1e-9
		m_PositionTrackingOn = self.LookUpElement("#Check_PositionTrackingOn").DLGGetValue()
		m_DefocusTrackingOn = self.LookUpElement("#Check_DefocusTrackingOn").DLGGetValue()
		// write settings
		string name = GetApplicationDirectory(4,1)+"LHC Tomo Dialog Settings.txt"
		...
		file.WriteFile(m_AcqShiftOn+"\n")
		file.WriteFile(m_AcqShift+"\n")
	...
	TagGroup ButtonPressed_Advanced(object self)
	{
		...
		TagGroup Check_PositionErrorWarningOn = DLGCreateCheckBox("Show warning if error >   ",m_PositionErrorWarningOn)
		...
		TagGroup Check_DefocusErrorWarningOn = DLGCreateCheckBox("Show warning if error >   ",m_DefocusErrorWarningOn)
		...
		Tab_PositionTracking.DLGAddElement(Box_PositionError.DLGTableLayOut(3,1,0))
		...
		Tab_DefocusTracking.DLGAddElement(Box_DefocusError.DLGTableLayOut(3,1,0))
		...
		if(DialogObject.Pose())
		{
			...
			if(m_i>0)
			{
				...
				if(OKCancelDialog("Re-calculate defocus?"))
				{
	...
	void ButtonPressed_StartStop(object self)
	{
		...
		//	object ViewParams = camera.CM_GetCameraAcquisitionParameterSet("Imaging","View","Focus",0)
			number xbin, ybin
			xbin = ybin = 4
		//	ViewParams.CM_GetBinning(xbin,ybin)
			if(xbin!=ybin)
			{
			...
			m_n = round((m_tStop-m_tStart)/m_dt)+1
			m_i = 1
			m_Corrected = 0
		...
		//	m_Img0 = camera.CM_CreateImageForAcquire(ViewParams,"")
			m_Img1 = m_Img0
			m_Img2 = m_Img0
			m_ImgF = m_Img0
			m_Img0.ShowImage()
		...
		//	number left = 140
			...
			//	image img3 := camera.CM_AcquireImage(ViewParams)
				// result
	...
	//	image img := camera.CM_AcquireImage(ViewParams)
	...
	//	object AcqParams = camera.CM_GetCameraAcquisitionParameterSet("Imaging","Acquire","Record",0)
		number DefocusCount
		image DefocusList := LHC_Val(self.LookUpElement("#Field_ImgDefocus").DLGGetStringValue(),DefocusCount)*1e-9
		m_RefDefocus = self.LookUpElement("#Field_RefDefocus").DLGGetValue()*1e-9
		number RefDefocusIndex = -1
		for(number i=0;i<DefocusCount;i++)
			if(abs(DefocusList.GetPixel(i,0)-m_RefDefocus)<m_DefocusAccuracy)
				RefDefocusIndex = i
		// obtain img0
		if(RefDefocusIndex<0 || m_AcqShiftOn)
		{
			img := OpenImage(GetApplicationDirectory(2,1)+"1.dm3")
		//	img := camera.CM_AcquireImage(ViewParams)
		}
		else
		{
			img := OpenImage(GetApplicationDirectory(2,1)+"1.dm3")
		//	img := camera.CM_AcquireImage(AcqParams)
			img.SaveImage(m_Folder+m_Prefix+decimal(m_i,4)+"DF"+round(DefocusList.GetPixel(RefDefocusIndex,0)*1e9)+"nm.dm3")
		}
		m_Img0 = img.LHC_Resize(m_w,m_h)
		// update progress
		self.LookUpElement("#Label_Progress").DLGTitle(""+m_i+" / "+m_n+"   viewed")
		...
		m_AcqShiftOn = self.LookUpElement("#Check_AcqShiftOn").DLGGetValue()
		m_AcqShift = self.LookUpElement("#Field_AcqShift").DLGGetValue()*1e-9
		number x = LHC_GetGonX()
		if(m_AcqShiftOn)
		{
			LHC_SetGonX(x+m_AcqShift)
			LHC_ResetBacklash(m_xb,0,0,0)
			sleep(m_Delay)
		}
		number FocusDistance = LHC_GetFocusDistance()
		for(number i=0;i<DefocusCount;i++)
			if(m_AcqShiftOn || i!=RefDefocusIndex)
				{
	...
	void Img0MouseClick(Object self, Number e_fl, ImageDisplay ImgDisp, Number r_fl, Number r_fl2, ROI r)
	{
		...
		else if(OKCancelDialog("Move goniometer?"))
		{
			self.RegulatePosition(px,py,m_a)
			m_cx += px
			m_cy += py
			self.SetElementIsEnabled("#Button_Next",0)
			if(OKCancelDialog("Goniometer has been moved. View now?"))
				self.ButtonPressed_View()
			else
				self.ResultLog()
		}
